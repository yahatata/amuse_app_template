...
**手動**の `linkOkibakeTemporaryEntryToBill` により、`open`/`in_progress` の bill と接続し、entry/addon/bust を後追い反映する。
...
後追い addon/bust/seat が bill 側と tournaments 側で整合  
...
- **`endTournament` / `validateEndTournament`**、および **`closeStoreTerminal` における `force_ended`**（現行コード確認済み）  
...
| 強制終了トリガ | `functions/src/domains/storeMeta/callables/closeStoreTerminal.ts`（§15 と整合させる処理を呼び出す／または共有サービス化） |
...
| Flutter | **要対応会計ページ**・`pending_review` カード: `lib/Accounting/` 配下および関連 VM（詳細書 §15.6。具体ファイルは **UI フェーズで列挙**） |
...
- bill 側から置きバケ由来の **entry / addon / bust / seat 反映を戻す**。**通常 bill / activeStay / 通常注文は削除しない**  
...
addon/bust 後追い済み、`open` と `settled` の可否  
...
- 誤紐付け解除 rollback（addon/bust 後追い込み済みケース）
