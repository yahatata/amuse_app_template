...
`新規Firebaseプロジェクト追加時_デプロイ手順.md` は、新しい店舗向けに Firebase Project ID を追加するときの外部操作（WIF/IAM/Secrets/Queue/デプロイ）をまとめた手順書である。
`中央管理アプリ連携手順.md` は、店舗側 deploy 後に中央管理アプリ `amuse-admin` へ監視対象として載せるための追加手順をまとめた手順書である。
