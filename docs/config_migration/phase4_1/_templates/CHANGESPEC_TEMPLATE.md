...
<!-- その段階で達成すること（1〜3行）。Flow2 の完了条件への対応を明記 -->
...
<!-- Flow2 4.2 の「確認すべき修正内容」を転記または要約。実装前に以下を確認すること。共通ルール: 必ず、依存先 step の完了サマリを確認し、必要に応じて実コードも確認する -->
...
<!-- Flow2 の「段階別参照ファイル」をベースに、実際の変更対象を特定 -->
...
<!-- 各ファイルの現状を簡潔に。該当コードの抜粋（行番号付き）があると実装時に便利 -->
