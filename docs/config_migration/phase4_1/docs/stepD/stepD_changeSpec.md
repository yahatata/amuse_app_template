...
- 退勤系 3 Callable に休憩中退勤時の処理を追加
- 詳細は [../phase_D/CHANGESPEC.md](../phase_D/CHANGESPEC.md) を参照
...
<!-- 共通ルール: 必ず、依存先 step の完了サマリを確認し、必要に応じて実コードも確認する。4.1-C の親再集計ヘルパーは stepC_completion_summary.md で実装内容を確認すること -->

| 依存先 | 確認すべき修正内容 |
|--------|-------------------|
| 4.1-B | clockOut, updateManualClockOutRecord, updateUnclockedAttendanceWithAuth の B による変更内容 |
| 4.1-C | 親再集計ヘルパー、breaks サブコレ、clockOut の C による挿入箇所。**stepC_completion_summary.md を確認** |
