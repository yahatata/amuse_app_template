...
| op-104 バスト＆再入場 | views/main（reentries+1, playersIn+1）、todaysBills（reentries+1, isBusted false）、tablesSeat（該当 seat の isBusted false, reentryCount+1） | main の reentries/playersIn を -1、todaysBills の reentries を -1・isBusted を true、tablesSeat をバスト状態に戻す。 | `tournamentId`, `actionLogId`, `playerUid`, `playerName`, `tableId`, `seatNumber` |
| op-105 座席割当 | views/main（seatedCount+1, waitingCount-1）、tablesSeat（該当 table に seat 追加）、tablesSeat/waiting（該当プレイヤー削除） | main の seatedCount -1、waitingCount +1。該当 table の seat を削除、waiting にプレイヤーを戻す。 | `tournamentId`, `actionLogId`, `playerUid`, `playerName`, `tableId`, `seatNumber` |
| op-106 全員着席替え | tablesSeat（全 table・waiting の座席配置を一括変更）、views/main（seatedCount, waitingCount） | **前状態の座席配置が必要**。previousSeatingData で tablesSeat を丸ごと復元し、main の統計を再計算。 | `tournamentId`, `actionLogId`, `previousSeatingData`（必須。waiting + 各 table.seats の前状態） |
