...
| # | functionEntry | 業務上の役割 | 主要業務 | 高頻度業務 | 主処理/補助 | FC静 |
|---|---------------|-------------|----------|-----------|-------------|-----------|
| 55 | `confirmShiftRequest` | ~~管理者からの希望シフト要請をスタッフが確認済みにする~~ **CLN-F2 ローカル削除済み**（production Function 削除待ち） | — | — | 廃止 |  |
| 56 | `createMultipleShifts` | ~~スタッフが複数日分のシフトを一括で申請する~~ **Batch 9 削除・undeploy 済（2026-09-03）**。正式経路は `submitShiftRequests` | — | — | 廃止 |  |
...
| 50 | `getRequiredStaffByTimeSlot` | `config_read` | storeMeta/businessStyles から必要人数を読み取る処理がリトライ後も失敗した際のログ | 3 | 1 | 主処理 |  | リトライ後も読み取れないときのみ `logOpsError`。fallback なし。必要人数未設定時は不足判定をスキップ |
