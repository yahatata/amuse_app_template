...
| R-07 | 給与締め日 (`PAYROLL_START_DAY`, `PAYROLL_END_DAY`, `PAYROLL_PERIOD_DESCRIPTION`) | `lib/globalConstant.dart` | `lib/AttendanceManagement/all_staff_attendance_page_from_adminHome.dart` | 店舗契約・締め日差分 | Run | `storeMeta/config.payroll.*` | Med | 締め日誤りで勤怠集計ミス |
