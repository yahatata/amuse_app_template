/**
 * recordPostSettlementCollection callable（Step04 changeSpec §3.2.5）。
 *
 * 仕様書 04_cashActions管理.md の later collection cashAction 作成 API。
 * `cashActionType` を 'collection' に固定して内部 repo に委譲する。
 */
...
export const recordPostSettlementCollection = onCall(async (request) => {
  if (!request.auth) {
  ...
  try {
    ...
    const idempotencyKey =
      ...
      `${billId}:recordPostSettlementCollection:${clientNonce ?? crypto.randomUUID()}`;
    ...
    logOpsError({
      message: 'recordPostSettlementCollection callable failed',
      functionEntry: 'recordPostSettlementCollection',
      operation: 'recordPostSettlementCollectionCallable',
      cause: error,
      context: { result: 'fail' },
    });
    ...
    throw new HttpsError(
      'internal',
      `recordPostSettlementCollection failed: ${
