...
/// ダイアログ用のセクション付きユーザーリストを構築
/// 
/// アルファベット（A-Z）→ 日本語（あ-ん）の順でソートし、
/// セクションヘッダー付きリストと左側インデックスを表示
Widget buildSectionedUserListDialog({
  ...
  // デフォルトのphysicsはClampingScrollPhysics（ダイアログ内用）
  final scrollPhysics = physics ?? const ClampingScrollPhysics();
  
  return _SectionedUserListDialogView(
    sections: sections,
...
/// セクション付きリストビュー（ダイアログ用、StatefulWidget）
class _SectionedUserListDialogView extends StatefulWidget {
  ...
  const _SectionedUserListDialogView({
  ...
  @override
  State<_SectionedUserListDialogView> createState() => _SectionedUserListDialogViewState();
}

class _SectionedUserListDialogViewState extends State<_SectionedUserListDialogView> {
