    ...
    try {
      ...
      if (response['success'] == true) {
        if (mounted) {
          ...
          // tournament_blind_template_list_page.dartに遷移
          Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
              builder: (context) => TournamentBlindTemplateList(
                tournamentTemplateId: widget.basicData['tournamentTemplateId'] ?? 'dummy',
                tournamentTemplateName: widget.basicData['tournamentTemplateName'] ?? 'ブラインドテンプレート管理',
              ),
            ),
            (route) => route.isFirst, // 最初の画面まで戻る
          );
        }
      } else {
        setState(() {
